...
class VirtMemoryRuntimeError (RuntimeError):
    pass

class VirtMemoryAccessError (RuntimeError):
    pass

class VirtMemory(HALBase):
    ...
    def read_virtual_mem( self, virt_address, length ):
        if self.logger.HAL: self.logger.log("[mem] 0x{:016X}".format(virt_address))
    ...
    def read_virtual_mem_dword( self, virt_address ):
        ...
        if self.logger.HAL: self.logger.log( '[mem] dword at VA = 0x{:016X}: 0x{:08X}'.format(virt_address, value) )
        return value
    ...
    def read_virtual_mem_word( self, virt_address ):
        ...
        if self.logger.HAL: self.logger.log( '[mem] word at VA = 0x{:016X}: 0x{:04X}'.format(virt_address, value) )
        return value
    ...
    def read_virtual_mem_byte( self, virt_address ):
        ...
        if self.logger.HAL: self.logger.log( '[mem] byte at VA = 0x{:016X}: 0x{:02X}'.format(virt_address, value) )
        return value
    ...
    def write_virtual_mem( self, virt_address, length, buf ):
        if self.logger.HAL:
            self.logger.log( '[mem] buffer len = 0x{:X} to VA = 0x{:016X}'.format(length, virt_address) )
            print_buffer( buf )
        phys_address = self.va2pa(virt_address)
        return self.helper.write_physical_mem( phys_address, length, buf )

    def write_virtual_mem_dword( self, virt_address, dword_value ):
        if self.logger.HAL: self.logger.log( '[mem] dword to VA = 0x{:016X} <- 0x{:08X}'.format(virt_address, dword_value) )
        phys_address = self.va2pa(virt_address)
    ...
    def write_virtual_mem_word( self, virt_address, word_value ):
        if self.logger.HAL: self.logger.log( '[mem] word to VA = 0x{:016X} <- 0x{:04X}'.format(virt_address, word_value) )
        phys_address = self.va2pa(virt_address)
    ...
    def write_virtual_mem_byte( self, virt_address, byte_value ):
        if self.logger.HAL: self.logger.log( '[mem] byte to VA = 0x{:016X} <- 0x{:02X}'.format(virt_address, byte_value) )
        phys_address = self.va2pa(virt_address)
    ...
    def free_virtual_mem(self, virt_address):
        ...
        if self.logger.HAL: self.logger.log( '[mem] Deallocated : VA = 0x{:016X}'.formatvirt_address )
