...
class Msr(HALBase):
    ...
    def get_cpu_thread_count( self ):
        ...
        if thread_count is None or thread_count < 0:
            if self.logger.HAL: self.logger.log( "helper.get_threads_count didn't return anything. Reading MSR 0x35 to find out number of logical CPUs (use CPUID Leaf B instead?)" )
        ...
        if self.logger.HAL: self.logger.log( "[cpu] # of logical CPUs: {:d}".format(thread_count) )
        return thread_count
...
##########################################################################################################
    ...
    def read_msr( self, cpu_thread_id, msr_addr ):
        ...
        if self.logger.HAL: self.logger.log( "[cpu{:d}] RDMSR( 0x{:x} ): EAX = 0x{:08X}, EDX = 0x{:08X}".format(cpu_thread_id, msr_addr, eax, edx) )
        return (eax, edx)

    def write_msr( self, cpu_thread_id, msr_addr, eax, edx ):
        self.helper.write_msr( cpu_thread_id, msr_addr, eax, edx )
        if self.logger.HAL: self.logger.log( "[cpu{:d}] WRMSR( 0x{:x} ): EAX = 0x{:08X}, EDX = 0x{:08X}".format(cpu_thread_id, msr_addr, eax, edx) )
        return
...
##########################################################################################################
    ...
    def get_IDTR( self, cpu_thread_id ):
        ...
        if self.logger.HAL:
            self.logger.log( "[cpu{:d}] IDTR Limit = 0x{:04X}, Base = 0x{:016X}, Physical Address = 0x{:016X}".format(cpu_thread_id,limit,base,pa) )
    ...
    def get_GDTR( self, cpu_thread_id ):
        ...
        if self.logger.HAL:
            self.logger.log( "[cpu{:d}] GDTR Limit = 0x{:04X}, Base = 0x{:016X}, Physical Address = 0x{:016X}".format(cpu_thread_id,limit,base,pa) )
        return (limit,base,pa)
...
##########################################################################################################
    ...
    def IDT( self, cpu_thread_id, num_entries=None ):
        if self.logger.HAL: self.logger.log( '[cpu{:d}] IDT:'.format(cpu_thread_id) )
    ...
    def GDT( self, cpu_thread_id, num_entries=None ):
        if self.logger.HAL: self.logger.log( '[cpu{:d}] GDT:'.format(cpu_thread_id) )
